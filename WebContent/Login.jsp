<%@ page language="java" contentType="text/html; charset=ISO-8859-1"
	pageEncoding="ISO-8859-1"%>
<!DOCTYPE html>
<html>
<head>
<meta charset="ISO-8859-1">
<title>Login</title>
<%@ include file="/Include/css.jsp"%>
</head>
<body>

	<%
		String pg =(String) request.getParameter("page");
		if(pg==null){
			pg =(String) request.getAttribute("page");
		}
		if(pg==null){
			pg="AfficherLivrePannierServlet";
		}
		
		Long ISBN;
		if (pg.equalsIgnoreCase("AjouterLivrePannierServlet")) {
			ISBN = Long.valueOf(Integer.parseInt(request.getParameter("ISBN")));
		} else {
			ISBN = null;
		}
	%>

	<div class="gtco-loader"></div>

	<div id="page">
		<%@ include file="/Include/Nav.jsp"%>
		<header id="gtco-header" class="gtco-cover gtco-cover-md"
			role="banner"
			style="background-image: url(Template/images/img_bg_2.jpg)">
			<div class="overlay"></div>
			<div class="gtco-container">
				<div class="row">
					<div class="col-md-12 col-md-offset-0 text-left">

						<div class="row row-mt-15em">
							<div class="col-md-7 mt-text animate-box"
								data-animate-effect="fadeInUp">
								<h1>Want to buy a book ?</h1>
							</div>
							<div class="col-md-4 col-md-push-1 animate-box"
								data-animate-effect="fadeInRight">
								<div class="form-wrap">
									<div class="tab">

										<div class="tab-content">
											<div class="tab-content-inner active" data-content="signup">

												<%
													if (ISBN != null) {
												%>

												<form
													action="LoginClientServlet?p=<%=pg%>&ISBN=<%=ISBN%>"
													method="post">
													<div class="row form-group">
														<div class="col-md-12">
															<label for="mail">Email</label> <input type="text"
																id="email" name="email" class="form-control">
														</div>
													</div>

													<div class="row form-group">
														<div class="col-md-12">
															<label for="password">Password</label> <input
																type="password" id="password" name="password"
																class="form-control">
														</div>
													</div>

													<div class="row form-group">
														<div class="col-md-12">
															<input type="submit" class="btn btn-primary btn-block"
																value="Submit">
														</div>
													</div>
												</form>
												<%
													} else {
												%>
												<form action="LoginClientServlet?p=<%=pg%>" method="post">
													<div class="row form-group">
														<div class="col-md-12">
															<label for="mail">Email</label> <input type="text"
																id="email" name="email" class="form-control">
														</div>
													</div>

													<div class="row form-group">
														<div class="col-md-12">
															<label for="password">Password</label> <input
																type="password" id="password" name="password"
																class="form-control">
														</div>
													</div>

													<div class="row form-group">
														<div class="col-md-12">
															<input type="submit" class="btn btn-primary btn-block"
																value="Submit">
														</div>
													</div>
												</form>
												<%
													}
												%>

											</div>

										</div>
									</div>
								</div>
							</div>
						</div>

					</div>
				</div>
			</div>
		</header>
	</div>
	<%@ include file="/Include/js.jsp"%>
</body>
</html>